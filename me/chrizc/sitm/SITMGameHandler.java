...
public class SITMGameHandler {
    ...
    public void startGame(Player player) {
        if (!plugin.inRegistration && !plugin.inGame) {
            if (plugin.checkPermissions("sitm.admin.reg", player)) {
    ...
    public void joinGame(Player player) {
        if (plugin.inRegistration && !plugin.inGame) {
            if (plugin.checkPermissions("sitm.users.join", player)) {
                if (plugin.players.containsKey(player) == false) {
                    ...
                    player.sendMessage(ERR_PREFIX + "You're already registered! To un-register, type /mud unreg");
                }
            } else {
                player.sendMessage(ERR_PREFIX + "You do not have permission to do that.");
            }
    ...
    public void beginGame(Player player, int timeLimit) {
        ...
        if (plugin.checkPermissions("sitm.admin.begin", player)) {
    ...
    public void cancelCountdown(Player player) {
        if (plugin.checkPermissions("sitm.admin.cancel", player)) {
            if (plugin.inCountdown) {
                Bukkit.getServer().getScheduler().cancelTasks(plugin);
                plugin.inGame = false;
                plugin.inCountdown = false;
    ...
    public void unTelePlayers(boolean clear) {
        ...
        if (clear) plugin.oldLocations.clear();
    }
